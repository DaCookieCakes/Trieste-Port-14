...
using Content.Shared._EmberFall.Bell;
...
public sealed partial class BellConsoleSystem : SharedBellConsoleSystem
{
    ...
    public override void Initialize()
    {
        ...
        // UI subscriptions
        Subs.BuiEvents<Shared._EmberFall.Bell.Components.BellConsoleComponent>(BellUiKey.Key,
            subs =>
            {
    ...
    private void OnOpened(Entity<Shared._EmberFall.Bell.Components.BellConsoleComponent> ent, ref BoundUIOpenedEvent args)
    {
    ...
    {
        if (!HasComp<Shared._EmberFall.Bell.Components.BellComponent>(platform))
            return;

        var query = EntityQueryEnumerator<Shared._EmberFall.Bell.Components.BellConsoleComponent>();
    ...
    private void UpdateUI(Entity<Shared._EmberFall.Bell.Components.BellConsoleComponent> ent)
    {
        ...
        var destinations = new List<Shared._EmberFall.Bell.Components.BellDestination>();

        // Get available destinations
        if (TryComp<Shared._EmberFall.Bell.Components.BellComponent>(platform, out var elevator))
    ...
    private void OnFTL(Entity<Shared._EmberFall.Bell.Components.BellConsoleComponent> ent, ref DockingConsoleFTLMessage args)
    {
        if (ent.Comp.Platform is not { } platform ||
            !TryComp<Shared._EmberFall.Bell.Components.BellComponent>(platform, out var elevator))
            return;
    ...
    private void UpdatePlatform(Entity<Shared._EmberFall.Bell.Components.BellConsoleComponent> ent)
    {
    ...
    {
        var query = EntityQueryEnumerator<Shared._EmberFall.Bell.Components.BellComponent>();
