        ...
        MapAtmosphereComponent atmos)
    {
        ...
        for (var x = bottomLeft.X; x <= topRight.X; x++)
        {
            for (var y = bottomLeft.Y; y <= topRight.Y; y++)
            {
                ...
                if (_mapManager.TryFindGridAt(args.MapId, tilePosition, out var gridUid, out var foundGrid))
                {
                    var tileRef = _mapSystem.GetTileRef(gridUid, foundGrid, new Vector2i((int)tilePosition.X, (int)tilePosition.Y));
